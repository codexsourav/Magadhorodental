...
function NewBlogs() {
    ...
    const saveData = async () => {
        try {
            ...
            const responseData: AxiosResponse = await makeRequest("/api/blog/" + id, "PUT", data);
            setLoading(false);
    ...
    const loadData = async () => {
        ...
        try {
            ...
            const data: IBlogData = responaseData.data;
            setBlogForm({
                title: data.title,
                image: data.image,
    ...
    return (
        <AdminWrapper>
            <h1 className="font-bold text-2xl">Update Blog Post</h1>
            ...
            <br />
            <Button onClick={handleSubmit} disabled={loading}>{loading ? "Loading.." : "Update Blog Post"}</Button>
        </AdminWrapper>
    );
}
